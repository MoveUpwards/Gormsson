...
class DetailViewController: UIViewController {
    ...
    @IBAction private func readStrings(_ sender: Any) {
        manager?.read(.manufacturerNameString, result: { result in
            switch result {
            case .success(let value):
    ...
    @IBAction private func freeMemory(_ sender: Any) {
        manager?.read(GPSFreeMemory(), result: { result in
            switch result {
            case .success(let value):
    ...
    @IBAction private func gpsStatusStartNotify(_ sender: Any) {
        manager?.read(GPSStatus(), result: { result in
            switch result {
            case .success(let value):
    ...
    @IBAction private func gpsSessionCount(_ sender: Any) {
        manager?.read(GPSSessionCount(), result: { result in
            switch result {
            case .success(let value):
    ...
    @IBAction private func gpsControlWriteStart(_ sender: Any) {
        manager?.write(GPSControl(), value: GPSControlEnum.start, result: { result in
            switch result {
            case .success(let value):
    ...
    @IBAction private func gpsControlWriteStop(_ sender: Any) {
        manager?.write(GPSControl(), value: GPSControlEnum.stop, result: { result in
            switch result {
            case .success(let value):
